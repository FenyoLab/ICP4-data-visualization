...
plot_server <- function(id, user_info, file_path){#, molecule_type) {
  moduleServer(id, function(input, output, session) {
    ...
    # plot_data = res data with additional label column for up-regulated, down-regulated, or non-signficant gene
    plot_data <- reactive({
    ...
    output$volcano_plot <- renderPlotly({
      ...
      plot_ly(
          plot_data(), 
          x = ~log2FoldChange, y = ~log10p, text = ~display_name,# text = ~paste(gene_name, display_id, sep = ":"),
